...
class Kasiski:
    def _repeated_seq_pos(self, text, seq_len):
    ...
    def _get_spacings(self, positions):
    ...
    def _candidate_key_lengths(self, factor_lists, max_key_len):
        all_factors = [factor_lists[lst][fac] for lst in range(len(factor_lists)) for fac in
                       range(len(factor_lists[lst]))]
    ...
    def find_key_length(self, ciphertext, seq_len, max_key_len):
        # find repeated sequences and their positions
        rsp = self._repeated_seq_pos(text=ciphertext, seq_len=seq_len)
        seq_spc = {}
        for seq, positions in rsp:
            seq_spc[seq] = self._get_spacings(positions)
        # calculate spacings between positions of each repeated
        # sequence and factor out spacings
        factor_lists = []
        for spacings in seq_spc.values():
            for space in spacings:
                factor_lists.append(self._get_factors(number=space))
